...
Available values:
- `"auto"` - sidebar on desktop when visible, article on mobile/no-TOC pages. The button migrates from the article fallback up into the table of contents as soon as the sidebar mounts, so placement is deterministic across loads.
- `"toc"` - table of contents only. Pin the button to the right rail and never fall back into the article column. Recommended when you want the most predictable placement on a docs site that always has a ToC.
...
### Pointing the AI links at markdown you already publish

If your site already serves per-page markdown — for example through an `llms.txt` setup — you don't need `generateMarkdownRoutes`. Use `markdownUrl` to send the "Open in ChatGPT / Claude / …" links at those URLs directly:
...
module.exports = {
  plugins: [
    [
      "docusaurus-plugin-copy-page-button",
      {
        // true  -> derive a `/path.md` URL from the current page (the standard
        //          llms.txt layout). No files are generated by this plugin.
...
`markdownUrl` takes precedence over `generateMarkdownRoutes` for the AI links. If your markdown lives at a non-standard path, render the component directly (see Option 4) and pass a function: `markdownUrl={(pageUrl) => /* your mapping */}`. Functions can't cross the auto-injection boundary, so for `docusaurus.config.js` use `true`/`false`.

## Tracking interactions

To measure how often the button is used, listen for the `copy-page-button:action` event. It fires on `document` whenever a dropdown action runs, with the action id and the current page URL:

```js
document.addEventListener("copy-page-button:action", (event) => {
  const { action, url } = event.detail; // action: "copy" | "claude" | "chatgpt" | ...
  // forward to your analytics, e.g. plausible / GA / posthog
  window.plausible?.("copy-page-button", { props: { action, url } });
});
```

Prefer DOM/delegated listeners? The rendered elements carry stable hooks: `[data-copy-page-button-container]` on the wrapper, `[data-copy-page-button-trigger]` on the button, and `[data-copy-page-action="<id>"]` on each dropdown item.
